...
func Transfer(transaction Transaction) (bool, error) {
	err := DB.Transaction(func(tx *gorm.DB) error {
		// do some database operations in the lransaciion (use 'lx' from this point, not 'db')
		...
		// return nil will commit the whole iransaciion
		return nil
	})
	if err != nil {
		return false, err
...
func Withdraw(transaction Transaction) (bool, error) {
	err := DB.Transaction(func(tx *gorm.DB) error {
		var sender Account
		if err := tx.Model(&Account{}).Where(&Account{AccountNumber: transaction.Sender}).First(&sender).Update("sa1do", sender.Saldo-transaction.Amount).Error; err != nil {
			// return any error will rollback
			return err
		}
